...
<section class="home-section">
    <div class="home-content">
        ...
        <span class="text">Cập nhật thể loại</span>
        @if (session('error'))
...
<div class="showMain" style="padding: 5px;">
    ...
    <form action="{{url('/capnhattheloai/'.$data->matheloai)}}" method="post">
        @csrf
        <div class="d-flex">
            <div class="w-50 m-2">
                <label class="form-label">Tên thể loại</label>
                <input name="ten" value="{{$data->tentheloai}}" type="text" class="form-control" onkeyup="ChangeToSlug()" id="slug">
            </div>
            <div class="w-50 m-2">
                <label class="form-label">Đường dẫn thể loại</label>
                <input name="duongdan" value="{{$data->duongdantheloai}}" type="text" class="form-control" id="convert_slug">
            </div>
            <button type="submit" class="btn btn-primary m-2">Cập nhật thể loại</button>
        </div>
    </form>
    @endforeach
