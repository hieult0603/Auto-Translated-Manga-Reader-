...
<div class="showMain" style="padding: 5px;">
    ...
    <form action="{{url('/themchap')}}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="d-flex">
            <div class="m-2 w-25">
                <label class="form-label">Số tập truyện</label>
                <input name="sochap" type="text" class="form-control">
            </div>
            <div class="m-2 w-75">
                ...
                <select name="truyen" class="form-select" aria-label="Default select example">
                    ...
                    <option value="{{$value->matruyen}}">{{$value->tentruyen}}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <button type="submit" class="btn btn-primary m-2">Thêm tập truyện</button>
    </form>
