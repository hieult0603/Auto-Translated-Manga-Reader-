...
<section class="home-section">
    <div class="home-content">
        ...
        <span class="text">Truyện đề cử</span>
        @if (session('error'))
                ...
                <tr>
                    ...
                    <td>
                        @if ($value->truyenhot == 1)
                            <i id="{{$value->matruyen}}" style="font-size:30px; color: #6092ff" class='bx bxs-star bodecutruyen'></i>
                        @else
                            <i id="{{$value->matruyen}}" style="font-size:30px; color: #6092ff" class='bx bx-star decutruyen'></i>
                        @endif
                    </td>
                </tr>
            @endforeach
...
<script>
    $(document).ready(function() {
        $('.decutruyen').click(function(){
            var matruyen = $(this).attr('id');
            let _token   = $('meta[name="csrf-token"]').attr('content');

            $.ajax({
                url:"{{url('/themdecutruyen')}}",
                method:"POST",
        ...
        $('.bodecutruyen').click(function(){
            var matruyen = $(this).attr('id');
            let _token   = $('meta[name="csrf-token"]').attr('content');

            $.ajax({
                url:"{{url('/bodecutruyen')}}",
                method:"POST",
                data:{matruyen:matruyen, _token:_token},
                success:function(){
                    location.reload();
